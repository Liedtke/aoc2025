...
merge = lambda op: operator.mul if op == "*" else operator.add
print(f"a = {sum(reduce(merge(op), operands) for operands, op in zip(zip(*nums), ops))}")
...
for num in nums:
    if num == "":
        ...
        curr.append(int(num))
parsed_nums.append(curr)
print(f"b = {sum(reduce(merge(op), operands) for operands, op in zip(parsed_nums, ops))}")
